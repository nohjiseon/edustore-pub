...
const OrganizationResetPasswordPage = () => {
  ...
  const onSubmit = async (data: FindPasswordFormData) => {
    console.log('기관회원 비밀번호 재설정 데이터:', data)

    try {
      await authService.sendOrganizationPasswordResetEmail(data.email)
      router.push('/signup/complete?type=sendLink')
    } catch (error: any) {
      console.error('기관회원 비밀번호 재설정 이메일 발송 실패:', error)
      // 에러 처리는 필요에 따라 추가
    }
  }
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.title_wrap}>
        <h1 className={styles.title}>기관회원 비밀번호 재설정</h1>
        <p className={styles.description}>
          수업가게에 가입한 이메일을 입력해주시면 <br />
          비밀번호 재설정 링크가 발송됩니다.
        </p>
      </div>
...
export default OrganizationResetPasswordPage
