...
// import { Props } from 'app/signup/complete/SignupCompletePage'
...
const OrderSummary = ({ data }: Props) => {
  ...
  // 세부 정보 배열 생성
  const details = [
    { label: '적용 학년', value: data.gradeSubjects[0].gradeNm },
    { label: '적용 교과', value: data.gradeSubjects[0].subjectNm },
    { label: '업로드 일', value: data.product.createDt?.split(' ')[0] },
    { label: '자료 형식', value: data.forms[0].formNm }
  ]
  ...
  const convertedTags: Tag[] = data.hashtags?.map((tag) => ({
    name: tag.hashtagNm,
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const memNo = Number(user.memNo)
      if (isNaN(memNo)) {
        alert('회원 번호가 유효하지 않습니다.')
        setIsAdding(false)
        return
      }
    ...
    >
      ...
      <div className={styles.header_section}>
        <div className={styles.content_wrapper}>
          ...
          {/* 제목 */}
          <h1 className={styles.title}>{data.product.productNm}</h1>

          {/* 작성자 정보 */}
          <div className={styles.author_info}>
            <Avatar
              name={data.memberView.nickname}
              // avatar={data.product.authorImage}
              size={22}
            />
          </div>
        </div>
